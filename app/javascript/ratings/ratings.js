...
class Ratings {
  ...
  initializeScore() {
    if (this.gaugeInitialScoreEl) {
      const score = parseInt(this.gaugeInitialScoreEl.value);
  ...
  // TODO: post form via buttons, grab the response
  // either fire custom event "scoreUpdateReceived"
  // and update UI on event. or just inject the new html from
  // the response here, and
  // update the gauge
  postRating() {
    fetch('/ratings', {
      method: 'POST'
